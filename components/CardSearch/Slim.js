  ...
  return (
    <Card bg="dark" text="light">
      <Card.Body className="py-1 ps-2 pe-0 d-flex align-items-center">
        ...
        <Button variant="link" size="sm" disabled={paused || !wordAvailable || search.status === 'mining'} onClick={() => purchaseWord(search.slug)} className="outline-0 shadow-none py-0 px-2"><i className="fs-5 bi bi-coin"></i></Button>
        ...
        <Button variant="link" className="outline-0 shadow-none py-0 px-2" onClick={() => onCloseClick(search.slug)}><i className="fs-5 bi bi-x-lg"></i></Button>
      </Card.Body>
    </Card>
  );
};
